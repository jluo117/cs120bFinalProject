...
void OLED_CS(uint8_t x) {
  bcm2835_gpio_write(oled_cs, x);
}

void OLED_DC(uint8_t x) {
  bcm2835_gpio_write(oled_dc, x);
}

void OLED_RST(uint8_t x)  {
  bcm2835_gpio_write(oled_rst, x);
}
...
void OLED_SCK(uint8_t x)  {
  bcm2835_gpio_write(oled_sck, x);
}

void OLED_DIN(uint8_t x)  {
  bcm2835_gpio_write(oled_din, x);
}
...
int Device_Init(void) {
  ...
  if(!bcm2835_spi_begin())  {
    printf("bcm2835 spi init failed !\r\n");
    return 1;
  }
  //SPI config
  ...
  bcm2835_gpio_fsel(oled_sck, BCM2835_GPIO_FSEL_OUTP);
  bcm2835_gpio_fsel(oled_din, BCM2835_GPIO_FSEL_OUTP);
